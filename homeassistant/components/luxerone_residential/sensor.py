...
async def async_setup_entry(
    ...
    async_add_entities(
        [
            ...
            LuxerOnePackageHistorySensor(hass, luxerOneClient, config_entry),
        ],
        True,
    )
...
class LuxerOnePackageHistorySensor(SensorEntity):
    """Gets the historical package information."""
    ...
    def __init__(
        ...
        """Initialize a LuxerOnePackageHistorySensor."""
        super().__init__()
        self._luxerOne_client = luxerOne_client
        self._hass = hass
        self._config_entry = config_entry
        self._attr_unique_id = self._config_entry.data[ID] + "history"
        self._attr_name = "Package History " + self._config_entry.data[NAME]
        self.entity_id = generate_entity_id(
            ENTITY_ID_FORMAT, name=self._attr_name, hass=hass
        )
        self._attr_extra_state_attributes = {}
        _LOGGER.debug(
            "Created new LuxerOnePackageHistorySensor with id %s", self._attr_unique_id
        )
    ...
    def _get_historic_packages(self) -> list[Package]:
        return self._luxerOne_client.get_package_history()
    ...
    async def async_update(self, **kwargs: Any) -> None:
        ...
        packages: list[HistoricalPackage] = []
        # package codes will be called out separate from the data for easy use.
        package_codes: list[str] = []
        package_data: dict[str, Any] = {}
        try:
            ...
            for package in packages:
                ...
                package_datum["labels"] = package.labels
                package_datum["picked_up"] = package.pickedup
                package_datum["resident"] = package.resident
                package_datum["signature"] = package.signature
                package_data[package.id] = package_datum
